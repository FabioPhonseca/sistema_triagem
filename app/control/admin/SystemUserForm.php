...
class SystemUserForm extends TPage
{
    ...
    public function __construct()
    {
        ...
        $cpf->placeholder = 'Informe o CPF do usuário';
        
        $idPessoa  = new TEntry('id_pessoa');
        ...
        $this->form->addFields( [new TLabel('IdPessoa')], [$idPessoa] );
        
        $this->form->addFields([new TLabel('CPF')], [$cpf]);
        $cpf->setExitAction(new TAction([$this, 'onBuscaPessoa']));
    ...
    public static function onBuscaPessoa($param)
    {
        if (empty($param['cpf'])) {
            return;
        }

        $cpf = preg_replace('/\D/', '', $param['cpf']);
        try {
            TTransaction::open('banco_atendimento');
            $pessoa = Pessoa::where('cpf', '=', str_pad($cpf, 11, '0', STR_PAD_LEFT))->first();
            if ($pessoa instanceof Pessoa) {
                // preenche numfunc
                $data = new stdClass;
                $data->id_pessoa = $pessoa->id;
                $data->name = $pessoa->nome;
                $data->email = $pessoa->email_particular;
                $data->phone = $pessoa->telefone;
                TForm::sendData('form_System_user', $data);
            } else {
                new TMessage('warning', 'Pessoa não encontrada. Cadastre uma nova pessoa.');
                SystemUserForm::onOpenNewPerson($cpf);            
            }
            TTransaction::close();
        } catch (Exception $e) {
    ...
    public static function onOpenNewPerson($cpf)
    {
        //$cpf = TSession::getValue(__CLASS__ . '_novo_cpf');

        $params = [
            'cpf'         => $cpf,
            'callerForm'  => 'form_System_user',        // que vai gerar "form_ComparecimentoForm"
